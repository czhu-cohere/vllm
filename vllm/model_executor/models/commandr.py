...
from transformers import CohereConfig, Cohere2Config
...
class CohereDecoderLayer(nn.Module):
    def __init__(
        ...
        self.input_layernorm = norm_cls(
            param_shape=(config.hidden_size), eps=norm_eps
        )
...
class Cohere2MoE(nn.Module):
    ...
    def __init__(self,
        ...
        if hasattr(config, 'expert_selection_fn'
        ...
        if hasattr(config,
                   'num_shared_experts') and config.num_shared_experts > 0:
...
class Cohere2MoeDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if self.layer_idx >= getattr(self.config, "first_k_dense_replace", 0):
            self.mlp = Cohere2MoE(config=config,
                                quant_config=quant_config,
                                prefix=f"{prefix}.mlp")
        else:
            self.mlp = CohereMLP(config=config,
                                 intermediate_size=self.config.prefix_dense_intermediate_size,
                                quant_config=quant_config,
                                prefix=f"{prefix}.mlp")

        norm_cls, norm_eps = select_norm_impl(config)
        self.input_layernorm = norm_cls(param_shape=(config.hidden_size),
                                             eps=norm_eps)
...
class Cohere2MoeModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        norm_cls, norm_eps = select_norm_impl(config)
        self.norm = norm_cls(param_shape=(config.hidden_size),
                              eps=norm_eps)
...
class Cohere2MoeForCausalLM(nn.Module, SupportsLoRA, SupportsPP, SupportsQuant):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.model = Cohere2MoeModel(vllm_config=vllm_config,
